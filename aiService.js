...
// aiService.js - 完整版（含重试、超时、所有游戏设定）
const axios = require('axios');
...
`;
  ...
  const makeRequest = async (retries = 2) => {
    try {
      const response = await axios.post(DEEPSEEK_API_URL, {
        model: 'deepseek-chat',   // 使用更稳定的 chat 模型，之后可改回 deepseek-reasoner
        messages,
        temperature: 0.7,
        max_tokens: 800,
      }, {
      ...
      if (retries > 0 && (error.code === 'ECONNRESET' || error.code === 'ETIMEDOUT' || error.response?.status >= 500)) {
        console.log(`API 请求失败，剩余重试次数: ${retries}，错误: ${error.code || error.message}`);
  ...
  try {
    ...
    console.error('DeepSeek API 错误:', error.code || error.message);
    return { reply: '世界暂时无法回应，请稍后再试。', playerDied: null, gameOver: false, winner: null };
  }
}
  ...
  const makeRequest = async (retries = 2) => {
    try {
      ...
      if (retries > 0 && (error.code === 'ECONNRESET' || error.code === 'ETIMEDOUT')) {
        console.log(`摘要请求失败，重试剩余: ${retries}`);
        await new Promise(resolve => setTimeout(resolve, 1000));
        return makeRequest(retries - 1);
      }
      throw error;
